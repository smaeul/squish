...
/* Generated by a modified version of my assn2.5 Huffman encoder from barbara.raw and lena.raw. */
...
flushbits(int fd, uint8_t *state)
{
	...
	if (cursize) {
		/* Shift *state left to put the data in the msb's. Remaining bits are zeroes... */
		*state <<= CHAR_BIT - cursize;
		if ((err = writeall(fd, state, 1)) < 0)
			goto out;
		/* The sentinel bit must be present for the next function call's log2u32. */
		*state = 1;
		/* Record the extra byte written. */
		totalwritten += sizeof(*state);
	}
	err = 0;
...
imagefile_read_huffman(int fd, size_t depth, struct image **img)
{
	...
	if ((err = image_alloc(&newimg, TEST_SIZE, TEST_SIZE, depth)) < 0)
		goto out;
	/* Read until we have filled in the entire image. */
	while (block < newimg->height * newimg->width / BLOCKPIXELS) {
		int32_t ac, dc;
		uint8_t acpixels = 0;

		/* Read the DC component of this block. */
		if ((err = read_dc(fd, &state, &dc)) < 0)
			goto out;
		newimg->data[BLOCKPIXELS * block] = dc;
		/* Read AC components until we get to the end of the block. */
...
imagefile_write_huffman(int fd, struct image *img)
{
	...
	for (size_t block = 0; block < img->height * img->width / (BLOCKPIXELS); block += 1) {
		...
		for (size_t i = 1; i < BLOCKPIXELS; i += 1) {
			/* If the value is zero, it could be part of a run of zeroes... */
			if (!img->data[BLOCKPIXELS * block + i]) {
				if (i == BLOCKPIXELS - 1) {
					/* Unless it is the last pixel, and then we write an EOB. */
				...
				/* To keep the Huffman tables small, runs are no longer than 15+1 pixels. */
				while (zeroes >= 16) {
				...
				if ((err = write_ac(fd, &state, zeroes, img->data[BLOCKPIXELS * block + i])) < 0)
					goto out;
				/* We've handled this run, so reset the count for the next one. */
				zeroes = 0;
			}
		}
	}
	/* Ensure there is nothing left in the *state variable. */
...
write_ac(int fd, uint8_t *state, uint8_t zeroes, int32_t value)
{
	...
	/* Find and write the Huffman tuple. */
	size = log2i32(value);
	addbits(bitstring, log2u32(huffman_ac[size][zeroes]), huffman_ac[size][zeroes]);
	/* Write the variable-length integer if it is present. */
	if (size) {
		if (value < 0)
			value += (1 << size) - 1;
...
writebits(int fd, uint8_t *state, uint32_t bitstring)
{
	...
	while (newsize) {
		...
		uint8_t copysize = min(sizeof(*state) * CHAR_BIT - cursize, newsize);
		...
		*state |= (bitstring >> (newsize - copysize)) & ((1U << copysize) - 1);
		...
		if (cursize == sizeof(*state) * CHAR_BIT) {
			if ((err = writeall(fd, state, sizeof(*state))) < 0)
				goto out;
			cursize = 0;
			/* The sentinel bit must be present for the next function call's log2u32. */
			*state = 1;
